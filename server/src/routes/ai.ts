...
aiRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { excelColumns, dealflowFields } = req.body;

      if (!Array.isArray(excelColumns) || !Array.isArray(dealflowFields)) {
        res
          .status(400)
          .json({ error: "excelColumns와 dealflowFields는 배열이어야 합니다" });
        return;
      }

      console.log(
        `[column-mapping] ${excelColumns.length} columns → ${dealflowFields.length} fields`
      );

      const result = await mapColumns({ excelColumns, dealflowFields });
      res.json(result);
    } catch (err) {
      next(err);
    }
